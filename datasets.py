...
class MedicalSegmentation1(Dataset):
    def __init__(self, tmp_dir, index_range, use_transforms):
        ...
        for url, file in zip(urls, files):
            if not os.path.isfile(f'{tmp_dir}/{file}'):
                gdown.download(url, f'{tmp_dir}/{file}', quiet=False)
        imgs_path = f'{tmp_dir}/tr_im.nii.gz'
        images = nib.load(imgs_path)
        self.images = images.get_fdata()[..., index_range]
        lesion_masks_path = f'{tmp_dir}/tr_mask.nii.gz'
        lesion_masks = nib.load(lesion_masks_path)
        self.lesion_masks = lesion_masks.get_fdata()[..., index_range]
        lung_masks_path = f'{tmp_dir}/tr_lungmasks_updated.nii.gz'
...
class MedicalSegmentation2(Dataset):
    def __init__(self, tmp_dir, index_volume, use_transforms):
        ...
        for url, file in zip(urls, files):
            zip_path = f'{tmp_dir}/{file}'
            if not os.path.isfile(zip_path):
                gdown.download(url, zip_path, quiet=False)
                with ZipFile(zip_path, 'r') as z:
                    z.extractall(f'{tmp_dir}')

        images_fullname_list = glob.glob(f'{tmp_dir}/rp_im/*.nii.gz')
        images_fullname_list.sort()
        images = nib.load(images_fullname_list[index_volume])
        self.images = images.get_fdata()

        lesion_masks_fullname_list = glob.glob(f'{tmp_dir}/rp_msk/*.nii.gz')
        ...
        lung_masks_fullname_list = glob.glob(f'{tmp_dir}/rp_lung_msk/*.nii.gz')
...
class CTSegBenchmark(Dataset):
    def __init__(self, tmp_dir, index_volume, index_range, use_transforms):
        ...
        for url, file in zip(urls, files):
            zip_file = f'{tmp_dir}/{file}.zip'
            if not os.path.isfile(zip_file):
                r = requests.get(url)
                with open(zip_file, 'wb') as f:
                    f.write(r.content)
                with ZipFile(zip_file, 'r') as z:
                    z.extractall(f'{tmp_dir}/{file}/')

        images = np.array([]).reshape(512, 512, 0)
        for fullname in glob.glob(f'{tmp_dir}/{files[0]}/*.nii.gz'):
        ...
        for fullname in glob.glob(f'{tmp_dir}/{files[1]}/*.nii.gz'):
        ...
        for fullname in glob.glob(f'{tmp_dir}/{files[2]}/*.nii.gz'):
