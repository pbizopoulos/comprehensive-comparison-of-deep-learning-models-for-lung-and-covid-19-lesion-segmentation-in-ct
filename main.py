...
def save_figure_weights(model, experiment_name, architecture_name, encoder_weights):
    ...
    for index_rows in range(rows):
        for index_columns in range(columns):
            ...
            ax.set_xticklabels([])
            ax.set_yticklabels([])
    plt.savefig(f'{tmpdir}/{experiment_name}-{architecture_name}-{encoder_weights}-weights')
    plt.close()
...
def save_figure_histogram(hist_images, hist_masks, hist_range, experiment_name):
    ...
    ax.set_yscale('log')
    ax.legend()
    plt.savefig(f'{tmpdir}/{experiment_name}-hist')
    plt.close()
...
def save_figure_loss(loss, train_or_validation, experiment_name, architecture_name_list, ylim):
    ...
    plt.savefig(f'{tmpdir}/{experiment_name}-{train_or_validation.lower().replace(" ", "-")}-loss')
    plt.close()
...
def save_figure_architecture_box(dice, architecture_name_list, experiment_name):
    ...
    plt.ylim([70, 100])
    plt.savefig(f'{tmpdir}/{experiment_name}-boxplot-dice')
    plt.close()
...
def save_figure_initialization_box(dice, encoder_weights_list):
    ...
    plt.ylim([70, 100])
    plt.savefig(f'{tmpdir}/initialization-boxplot-dice')
    plt.close()
...
def save_figure_scatter(num_parameters_array, dice, architecture_name_list, experiment_name, ylim):
    ...
    plt.savefig(f'{tmpdir}/{experiment_name}-scatter-dice-vs-num-parameters')
    plt.close()
...
def save_figure_3d(volume, is_full, experiment_name, architecture, encoder_weights):
    ...
    for line in ax.zaxis.get_ticklines():
        line.set_visible(False)
    plt.savefig(f'{tmpdir}/{experiment_name}-{architecture}-{encoder_weights}-volume')
    plt.close()
...
def save_figure_image(image, experiment_name):
    ...
    plt.savefig(f'{tmpdir}/{experiment_name}-image')
    plt.close()
...
def save_figure_image_masked(image, mask, prediction, experiment_name, architecture, encoder):
    ...
    plt.savefig(f'{tmpdir}/{experiment_name}-{architecture}-{encoder.replace("_", "")}-masked-image')
    plt.close()
...
class MedicalSegmentation1(Dataset):
    def __init__(self, index_range, use_transforms):
        ...
        for url, file in zip(urls, files):
            if not os.path.isfile(f'{tmpdir}/{file}'):
                gdown.download(url, f'{tmpdir}/{file}', quiet=False)
        imgs_path = f'{tmpdir}/tr_im.nii.gz'
        images = nib.load(imgs_path)
        self.images = images.get_fdata()[..., index_range]
        lesion_masks_path = f'{tmpdir}/tr_mask.nii.gz'
        lesion_masks = nib.load(lesion_masks_path)
        self.lesion_masks = lesion_masks.get_fdata()[..., index_range]
        lung_masks_path = f'{tmpdir}/tr_lungmasks_updated.nii.gz'
...
class MedicalSegmentation2(Dataset):
    def __init__(self, index_volume, use_transforms):
        ...
        for url, file in zip(urls, files):
            zip_path = f'{tmpdir}/{file}'
            if not os.path.isfile(zip_path):
                gdown.download(url, zip_path, quiet=False)
                with ZipFile(zip_path, 'r') as z:
                    z.extractall(tmpdir)
        images_fullname_list = glob.glob(f'{tmpdir}/rp_im/*.nii.gz')
        images_fullname_list.sort()
        images = nib.load(images_fullname_list[index_volume])
        self.images = images.get_fdata()
        lesion_masks_fullname_list = glob.glob(f'{tmpdir}/rp_msk/*.nii.gz')
        ...
        lung_masks_fullname_list = glob.glob(f'{tmpdir}/rp_lung_msk/*.nii.gz')
...
class CTSegBenchmark(Dataset):
    def __init__(self, index_range, use_transforms):
        ...
        for url, file in zip(urls, files):
            zip_file = f'{tmpdir}/{file}.zip'
            if not os.path.isfile(zip_file):
                r = requests.get(url)
                with open(zip_file, 'wb') as f:
                    f.write(r.content)
                with ZipFile(zip_file, 'r') as z:
                    z.extractall(f'{tmpdir}/{file}/')
        images = np.array([]).reshape(512, 512, 0)
        for fullname in glob.glob(f'{tmpdir}/{files[0]}/*.nii.gz'):
        ...
        for fullname in glob.glob(f'{tmpdir}/{files[1]}/*.nii.gz'):
        ...
        for fullname in glob.glob(f'{tmpdir}/{files[2]}/*.nii.gz'):
        ...
        for index_architecture, (architecture, architecture_name) in enumerate(zip(architecture_list, architecture_name_list)):
            for index_encoder, encoder in enumerate(encoder_list):
                for index_encoder_weights, encoder_weights in enumerate(encoder_weights_list):
                    ...
                    model_path = f'{tmpdir}/{experiment_name}-{architecture_name}-{encoder}-{encoder_weights}.pt'
