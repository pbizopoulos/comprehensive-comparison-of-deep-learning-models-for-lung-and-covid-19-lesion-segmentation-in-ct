...
if __name__ == '__main__':
    ...
    train_dataset = MedicalSegmentation1(cache_dir, index_train_range, use_transforms=True)
    train_dataloader = DataLoader(train_dataset, batch_size=batch_size, shuffle=True)
    validation_dataset = MedicalSegmentation1(cache_dir, index_validation_range, use_transforms=False)
        ...
        for index_architecture, (architecture, architecture_name) in enumerate(zip(architecture_list, architecture_name_list)):
            for index_encoder, encoder in enumerate(encoder_list):
                for index_encoder_weights, encoder_weights in enumerate(encoder_weights_list):
                    ...
                    model_path = f'{results_dir}/{experiment_name}-{architecture_name}-{encoder}-{encoder_weights}.pt'
                    for index_epoch, epoch in enumerate(range(num_epochs)):
                        ...
                        for images, lung_masks, lesion_masks in train_dataloader:
                            ...
                            if (architecture_name == architecture_name_list[0]) and (encoder == encoder_list[0]) and (encoder_weights == encoder_weights_list[0]) and (index_epoch == num_epochs - 1):
                                save_image(images[0, 0], experiment_name, results_dir)
                                save_masked_image(images[0, 0], masks[0, 0], masks[0, 0], experiment_name, 'mask', 'train', results_dir)
                                save_masked_image(images[0, 0], masks[0, 0], predictions[0, 0], experiment_name, 'prediction', 'train', results_dir)
                            if (architecture_name == architecture_name_list[0]) and (encoder == 'resnet18'):
                                if (index_epoch == 0):
                                    save_weights(model, experiment_name, architecture_name, f'{encoder_weights}-before', results_dir)
                                elif (index_epoch == num_epochs - 1):
                                    save_weights(model, experiment_name, architecture_name, f'{encoder_weights}-after', results_dir)
                    ...
                    for index_test_volume in index_test_volume_range:
                        test_dataset = MedicalSegmentation2(cache_dir, index_test_volume, use_transforms=False)
                        ...
                        with torch.no_grad():
                            ...
                            if index_test_volume == 0:
                                ...
                                save_masked_image(images[0], masks[0], predictions[0, 0], experiment_name, architecture_name, encoder, results_dir)
                            if (index_test_volume == 0) and (encoder == 'resnet18') and (encoder_weights == None):
                                ...
                                if architecture_name == 'Unet':
                                    save_3d(volume_mask, args.full, experiment_name, 'mask', '', results_dir)
                                volume_prediction = volume_prediction[:, :, ::-1]
                                save_3d(volume_prediction, args.full, experiment_name, architecture_name, encoder_weights, results_dir)

    for hist_images, hist_masks, experiment_name in zip(hist_images_array, hist_masks_array, experiment_name_list):
        save_hist(hist_images, hist_masks, hist_range, experiment_name, results_dir)

    for experiment_name, train_loss, validation_loss in zip(experiment_name_list, train_loss_array, validation_loss_array):
        save_loss(train_loss, 'Train', experiment_name, architecture_name_list, [0, 1], results_dir)
        save_loss(validation_loss, 'Validation', experiment_name, architecture_name_list, [0, 1], results_dir)
    save_loss(train_loss_array[2] - train_loss_array[1], 'Train diff', experiment_name, architecture_name_list, [-0.4, 0.4], results_dir)
    save_loss(validation_loss_array[2] - validation_loss_array[1], 'Validation diff', experiment_name, architecture_name_list, [-0.4, 0.4], results_dir)
    ...
    for experiment, experiment_name, metrics_ in zip(experiment_list, experiment_name_list, metrics_array):
        save_architecture_box(metrics_[..., -1], architecture_name_list, experiment_name, results_dir)
        save_scatter(num_parameters_array, metrics_[..., -1], architecture_name_list, experiment_name, [70, 100], results_dir)
    save_scatter(num_parameters_array, metrics_array[2, ..., -1] - metrics_array[1, ..., -1], architecture_name_list, 'diff', [-15, 15], results_dir)
    save_initialization_box(metrics_array[..., -1], encoder_weights_list, results_dir)
    ...
    df.to_latex(f'{results_dir}/metrics.tex', formatters=formatters, bold_rows=True, column_format='c|l|rrr|rrr|rrr|rrr|rrr|rrr|rrr', multirow=True, multicolumn=True, multicolumn_format='c', escape=False)
