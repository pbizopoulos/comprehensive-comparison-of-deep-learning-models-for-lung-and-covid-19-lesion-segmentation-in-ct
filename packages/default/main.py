#!/usr/bin/env python3
# Copyright (c) 2024 Paschalis Bizopoulos
...
class _MedicalSegmentation2(Dataset):  # type: ignore[misc]
    def __init__(
...
def _save_figure_3d(
    ...
    for line in ax.zaxis.get_ticklines():
        line.set_visible(False)
    plt.savefig(
        _OUT_PATH / f"{experiment_name}-{architecture}-{encoder_weights}-volume.png",
    )
    plt.close()
...
def _save_figure_histogram(
    ...
    ax.set_yscale("log")
    ax.legend()
    plt.savefig(_OUT_PATH / f"{experiment_name}-hist.png")
    plt.close()
...
def _save_figure_image(experiment_name: str, image: torch.Tensor) -> None:
    ...
    plt.savefig(_OUT_PATH / f"{experiment_name}-image.png")
    plt.close()
...
def _save_figure_image_masked(  # noqa: PLR0913
    *,
    ...
    plt.savefig(
        _OUT_PATH
        / f"{experiment_name}-{architecture}-{encoder_name.replace('_', '')}-masked-image.png",  # noqa: E501
    )
    plt.close()
...
def _save_figure_loss(
    ...
    plt.savefig(
        _OUT_PATH
        / f"{experiment_name}-{train_or_validation.lower().replace(' ', '-')}-loss.png",
    )
    plt.close()
...
def _save_figure_scatter(
    ...
    plt.savefig(
        _OUT_PATH / f"{experiment_name}-scatter-dice-vs-num-parameters.png",
    )
    plt.close()
...
def _save_figure_weights(
    ...
    plt.savefig(
        _OUT_PATH
        / f"{experiment_name}-{architecture_name}-{encoder_weights}-weights.png",
    )
    plt.close()
...
def main() -> None:  # noqa: C901,PLR0912,PLR0915
    ...
    for experiment_name_index, experiment_name in enumerate(experiment_names):
        for architecture_index, (architecture, architecture_name) in enumerate(
            zip(architectures, architecture_names, strict=True),
        ):
            for encoder_name_index, encoder_name in enumerate(encoder_names):
                for encoder_weights_index, encoder_weights in enumerate(
                    ...
                    flops = FlopCountAnalysis(
                        model,
                        dataloader_train.dataset[0][0].unsqueeze(0).to(device),
                    )
                    flops_array[
                    ...
                    for epoch_index in range(num_epochs):
                        ...
                        with torch.no_grad():
                            ...
                            loss_validation = loss_validation_sum / len(
                            ...
                            if (
                                ...
                                for slice_volume_index, (
                                    ...
                                    if experiment_name == experiment_names[0]:
                                        ...
                                        images *= mask_lungs  # noqa: PLW2901
                                        masks = mask_lesions
                                    volume_mask_array[:, :, slice_volume_index] = masks[
                                        0
                                    ].float()
                                    predictions = model(images.unsqueeze(0).to(device))
                                ...
                                volume_prediction_array = volume_prediction_array[
                                    :,
                                    :,
                                    ::-1,
                                ]
    ...
    encoder_mean = (
        metrics_array.transpose([2, 0, 1, 3, 4])
        .reshape(metrics_array.shape[2], -1)
        .mean(1)
    )
    ...
    index = pd.MultiIndex.from_product(
        [
            architecture_names,
            [encoder_name.replace("_", "") for encoder_name in encoder_names]
            + ["Mean"],
        ],
    )
    multicolumn = pd.MultiIndex.from_product(
        [
            [str(encoder_weights) for encoder_weights in encoders_weights],
            experiments,
            metric_names,
        ],
    )
    ...
    keys = (
        ...
        + [
            f"{experiment_name}-{encoder_weights}-mean"
            for experiment_name, encoder_weights in itertools.product(
                experiment_names,
                encoders_weights,
            )
        ]
        + [
            f"{experiment_name}-{encoder_weights}-max"
            for experiment_name, encoder_weights in itertools.product(
                experiment_names,
                encoders_weights,
            )
        ...
        + [
            f"{encoder_weights}-{stat}"
            for encoder_weights, stat in itertools.product(encoders_weights, ["mean"])
        ]
        + [
            f"{experiment_name}-architecture-{encoder_weights}-index-max"
            for experiment_name, encoder_weights in itertools.product(
                experiment_names,
                encoders_weights,
            )
        ...
        + [
            f"{experiment_name}-encoder-{encoder_weights}-index-max"
            for experiment_name, encoder_weights in itertools.product(
                experiment_names,
                encoders_weights,
            )
        ...
        + [
            f"{experiment_name}-{architecture_name}-{encoder_weights}-mean"
            for experiment_name, architecture_name, encoder_weights in itertools.product(  # noqa: E501
                experiment_names,
                architecture_names,
                encoders_weights,
            )
    ...
    values = (
        ...
        + [
            metrics_df.loc["Global", "Mean"][str(encoder_weights), experiment]["Dice"]
            for experiment, encoder_weights in itertools.product(
                experiments,
                encoders_weights,
            )
        ]
        ...
        + [
            ...
            for experiment, architecture_name, encoder_weights in itertools.product(
                experiments,
                architecture_names,
                encoders_weights,
            )
